...
function getAddr(name, ens, template, callback) {
  ...
  var resolverContract = new web3.ser.Contract(resolverContractAbi);
...
Template.dapp_addressInput.onCreated(function() {
  ...
  var ensContract = new web3.ser.Contract(ensContractAbi, ensAddress);
  ...
  web3.ser.isSyncing(function(err, syncing) {
    if (!err && !syncing) {
      // cannot use ENS while syncing
      web3.ser.getCode(ensAddress, function(err, code) {
